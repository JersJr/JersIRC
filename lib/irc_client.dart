...
class IrcMessage {
  ...
  static IrcMessage parse(String raw) {
    ...
    while (line.isNotEmpty) {
      ...
      if (line.startsWith(':')) { parts.add(line.substring(1)); break; }
      final space = line.indexOf(' ');
      if (space < 0) { parts.add(line); break; }
...
class IrcClient {
  ...
  Future<void> connect({required String host, required int port, required String nickname, String? username, bool secure = true}) async {
    ...
    _socket = socket;
    _connected = true;
    _subscription = socket.transform(utf8.decoder).transform(const LineSplitter()).listen(_handleLine,
      onDone: () { _connected = false; _messages.add(const IrcMessage('', null, 'DISCONNECTED', [])); },
      onError: (Object e) { _connected = false; _messages.add(IrcMessage(e.toString(), null, 'ERROR', [e.toString()])); });
    send('CAP LS 302');
    send('NICK $nickname');
  ...
  void _handleLine(String line) {
    ...
    if (m.command == 'PING') send('PONG :${m.trailing}');
    _messages.add(m);
  }
  ...
  Future<void> disconnect() async {
    ...
    _subscription = null;
    try { _socket?.write('QUIT :Leaving JersIRC\r\n'); } catch (_) {}
