...
class IrcMessage {
  ...
  static IrcMessage parse(String raw) {
    ...
    final command=parts.isEmpty?'':parts.removeAt(0).toUpperCase(); return IrcMessage(raw,prefix,command,parts);
  }
}
class IrcClient {
  ...
  Future<void> connect({required String host,required int port,required String nickname,String? username,bool secure=true})async{
    await disconnect(); final socket=secure?await SecureSocket.connect(host,port,timeout:const Duration(seconds:15)):await Socket.connect(host,port,timeout:const Duration(seconds:15)); _socket=socket;_connected=true;
    _subscription=socket.transform(utf8.decoder).transform(const LineSplitter()).listen(_handleLine,onDone:(){_connected=false;_messages.add(const IrcMessage('',null,'DISCONNECTED',[]));},onError:(Object e){_connected=false;_messages.add(IrcMessage(e.toString(),null,'ERROR',[e.toString()]));});
  ...
  void join(String channel)=>send('JOIN $channel'); void part(String channel,[String? reason])=>send('PART $channel${reason==null?'':' :$reason'}'); void changeNick(String nickname)=>send('NICK $nickname'); void message(String target,String text)=>send('PRIVMSG $target :$text'); void notice(String target,String text)=>send('NOTICE $target :$text'); void quit([String reason='Leaving JersIRC'])=>send('QUIT :$reason');
