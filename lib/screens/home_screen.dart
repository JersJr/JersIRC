...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void _command(String raw) {
    ...
    switch (cmd) {
      ...
      case 'notice':
        if (args.length >= 2) {
          controller.sendNotice(args.first, args.skip(1).join(' '));
        }
        break;
      case 'me':
        if (room != null && args.isNotEmpty) {
          controller.sendAction(room!.name, args.join(' '));
        }
        break;
      case 'query':
  ...
  List<IrcUserView> _sortedUsers(ChatRoomModel r) {
    final list = r.users.values
        .map((u) => IrcUserView(u.nick, u.prefixes))
        .toList();
    list.sort((a, b) {
          ...
          children: [
            ...
            Expanded(
              child: Row(
                children: [
                  Expanded(
                    child: room == null ? _welcome() : _chat(room!),
                  ),
          ...
          child: ListView(
            ...
            children: [
              ...
              SwitchListTile(
                ...
                value: secure,
                onChanged: controller.connecting
                    ? null
                    : (v) => setState(() => secure = v),
              ),
              FilledButton.icon(
                onPressed: controller.connecting
                    ? null
                    : (controller.connected ? controller.disconnect : _connect),
                icon: Icon(
                  controller.connected ? Icons.link_off : Icons.link,
                ),
                label: Text(
                  controller.connected ? 'Desconectar' : 'Conectar',
                ),
              ),
              ...
              if (savedServers.isEmpty)
                ...
                ...savedServers.map(
                  (s) => ListTile(
                    ...
                    contentPadding: EdgeInsets.zero,
                    leading: Icon(
                      s.tls ? Icons.lock_outline : Icons.public,
                    ),
                    title: Text(
                      s.name,
                      overflow: TextOverflow.ellipsis,
              ...
              Row(
                children: [
                  Expanded(
                    child: TextField(
                      controller: channel,
                      decoration: const InputDecoration(hintText: '#panama'),
                    ),
                  ),
                  const SizedBox(width: 8),
                  IconButton.filled(
                    onPressed: controller.connected
                        ? () => controller.join(channel.text)
                        : null,
  ...
  Widget _welcome() => Center(
        child: Column(
          ...
          children: [
            ...
            const Text(
              'Bienvenido a JersIRC',
              style: TextStyle(fontSize: 24, fontWeight: FontWeight.w700),
            ),
            const SizedBox(height: 8),
            const Text(
              'Conecta a un servidor IRC para comenzar',
              style: TextStyle(color: Colors.white54),
            ),
          ],
        ),
  ...
  Widget _tabs() => SizedBox(
        ...
        child: ListView(
          ...
          children: controller.rooms.values
              .map(
                (r) => Padding(
                  padding: const EdgeInsets.symmetric(horizontal: 2),
                  child: Material(
                    color: r.name == controller.activeRoom
                        ? const Color(0xFF53677D)
  ...
  Widget _tabTitle(ChatRoomModel r) {
    ...
    return _BlinkingUnread(
      label: label,
      count: r.unread,
      color: _nickColor(r.name),
    );
  }
                ...
                children: _sortedUsers(r)
                    .map(
                      (u) => ListTile(
                        ...
                        leading: Text(
                          u.prefixes.isEmpty ? '•' : u.prefixes,
                        ),
                        title: Text(
                          u.nick,
                          overflow: TextOverflow.ellipsis,
                          style: TextStyle(
                            color: _nickColor(u.nick),
...
class _BlinkingUnreadState extends State<_BlinkingUnread>
  ...
  Widget build(BuildContext context) => AnimatedBuilder(
        ...
        builder: (_, __) {
          ...
          final text = widget.label is Text
