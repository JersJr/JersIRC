...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void _send() {
    ...
    if (text.startsWith('/')) _command(text.substring(1));
  ...
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(
      ...
      actions: [Padding(padding: const EdgeInsets.only(right: 12), child: Row(children: [Icon(Icons.circle, size: 9, color: controller.connected ? const Color(0xFF8FB59B) : Colors.grey), const SizedBox(width: 6), SizedBox(width: 150, child: Text(controller.status, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 12)))]))],
    ),
    drawer: _drawer(),
  ...
  Widget _drawer() => Drawer(
    ...
    child: SafeArea(child: ListView(padding: const EdgeInsets.all(16), children: [
      ...
      if (savedServers.isEmpty) const Padding(padding: EdgeInsets.symmetric(vertical: 10), child: Text('No hay perfiles guardados', style: TextStyle(color: Colors.white54)))
      else ...savedServers.map((s) => ListTile(selected: selectedProfile == s.name, contentPadding: EdgeInsets.zero, leading: Icon(s.tls ? Icons.lock_outline : Icons.public), title: Text(s.name, overflow: TextOverflow.ellipsis), subtitle: Text('${s.host}:${s.port} • ${s.nickname}', overflow: TextOverflow.ellipsis), onTap: () { _applyProfile(s); Navigator.pop(context); }, trailing: PopupMenuButton<String>(onSelected: (v) { if (v == 'edit') _saveProfile(existing: s); if (v == 'delete') _deleteProfile(s); }, itemBuilder: (_) => const [PopupMenuItem(value: 'edit', child: Text('Editar')), PopupMenuItem(value: 'delete', child: Text('Eliminar'))]))),
      const Divider(height: 28, color: Colors.white12),
  ...
  Widget _chat(ChatRoomModel r) => Column(children: [
    if (!controller.connected) Container(width: double.infinity, padding: const EdgeInsets.all(6), color: Colors.orange.withOpacity(.12), child: Text(controller.status, style: const TextStyle(fontSize: 12))),
          ...
          itemBuilder: (context, i) {
            ...
            return Padding(
              ...
              child: RichText(text: TextSpan(children: [
                WidgetSpan(alignment: PlaceholderAlignment.middle, child: InkWell(onTap: user == 'Sistema' ? null : () => _showUserActions(user), borderRadius: BorderRadius.circular(4), child: Padding(padding: const EdgeInsets.symmetric(horizontal: 2, vertical: 1), child: Text('$user:', style: TextStyle(fontWeight: FontWeight.bold, color: _nickColor(user))))),
  ...
  Widget _users(ChatRoomModel r) => Container(
    ...
    child: Column(children: [
      Padding(padding: const EdgeInsets.fromLTRB(7, 5, 4, 2), child: Row(children: [Expanded(child: Text('USUARIOS · ${r.users.length}', style: const TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.white54))), IconButton(onPressed: () => setState(() => usersVisible = false), icon: const Icon(Icons.keyboard_double_arrow_right, size: 17), padding: EdgeInsets.zero, constraints: const BoxConstraints(minWidth: 28, minHeight: 28))])),
      Expanded(child: ListView(padding: const EdgeInsets.all(6), children: _sortedUsers(r).map((u) => ListTile(dense: true, contentPadding: const EdgeInsets.symmetric(horizontal: 2), leading: Text(u.prefixes.isEmpty ? '•' : u.prefixes), title: Text(u.nick, overflow: TextOverflow.ellipsis, style: TextStyle(color: _nickColor(u.nick), fontWeight: FontWeight.w600)), onTap: () => _showUserActions(u.nick))).toList())),
    ]),
  );
  ...
  Widget _collapsedUsers(ChatRoomModel r) => Material(
    ...
    child: InkWell(
      ...
      child: SizedBox(width: 44, child: Column(mainAxisAlignment: MainAxisAlignment.start, children: [const SizedBox(height: 8), const Icon(Icons.people_alt_outlined, size: 20), const SizedBox(height: 4), Text('${r.users.length}', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 12))])),
    ),
  );
  ...
  Color _nickColor(String user) {
    ...
    const c = [Color(0xFF7CB8FF), Color(0xFFFFA6C9), Color(0xFFB9E986), Color(0xFFFFCC80), Color(0xFFC7A7FF), Color(0xFF72E0D1), Color(0xFFFF9E80), Color(0xFF9FA8DA)];
    return c[h % c.length];
  }
}
...
class _BlinkingUnreadState extends State<_BlinkingUnread> with SingleTickerProviderStateMixin {
  ...
  @override Widget build(BuildContext context) => AnimatedBuilder(
    ...
    builder: (_, __) {
      ...
      return Row(mainAxisSize: MainAxisSize.min, children: [Text(widget.label is Text ? (widget.label as Text).data ?? '' : '', style: TextStyle(fontSize: 12, color: c, fontWeight: FontWeight.w700)), const SizedBox(width: 4), Text('(${widget.count})', style: TextStyle(fontSize: 12, color: c, fontWeight: FontWeight.bold))]);
    },
  );
}
