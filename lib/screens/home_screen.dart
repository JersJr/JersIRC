...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _saveProfile({SavedServer? existing}) async {
    ...
    final p = int.tryParse(port.text.trim()) ?? (secure ? 6697 : 6667); final ch = channel.text.trim(); final profile = SavedServer(name: name, host: host.text.trim(), port: p, nickname: nick.text.trim(), tls: secure, channels: ch.isEmpty ? const [] : [ch]);
  ...
  Future<void> _deleteProfile(SavedServer profile) async { final ok = await showDialog<bool>(context: context, builder: (_) => AlertDialog(title: const Text('Eliminar perfil'), content: Text('¿Eliminar "${profile.name}"?'), actions: [TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Cancelar')), FilledButton(onPressed: () => Navigator.pop(context, true), child: const Text('Eliminar'))])); if (ok != true) return; await storage.deleteServer(profile.name); if (await storage.getLastServer() == profile.name) await storage.clearLastServer(); final list = await storage.loadServers(); if (!mounted) return; setState(() { savedServers = list; if (selectedProfile == profile.name) selectedProfile = null; }); }
  ...
  Widget _tabTitle(ChatRoomModel r) { final label = Text(r.name, style: const TextStyle(fontSize: 12)); if (r.unread <= 0 || r.name == controller.activeRoom) return label; return _BlinkingUnread(label: label, count: r.unread); }
...
class _BlinkingUnread extends StatefulWidget {
  final Widget label; final int count;
  const _BlinkingUnread({required this.label, required this.count});
  @override State<_BlinkingUnread> createState() => _BlinkingUnreadState();
}
class _BlinkingUnreadState extends State<_BlinkingUnread> with SingleTickerProviderStateMixin {
  ...
  @override Widget build(BuildContext context) => FadeTransition(opacity: Tween<double>(begin: .3, end: 1).animate(_controller), child: Row(mainAxisSize: MainAxisSize.min, children: [widget.label, const SizedBox(width: 4), Text('(${widget.count})', style: const TextStyle(fontWeight: FontWeight.bold))]));
}
