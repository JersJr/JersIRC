...
class IrcController extends ChangeNotifier {
  ...
  bool _isPermanentConnectionError(String value) {
    ...
    return x.contains('nickname en uso') || x.contains('nickname no válido') || x.contains('contraseña incorrecta') || x.contains('rechazada/bloqueada');
  }

  void scheduleReconnect({required String host, required int port, required String nickname, required bool secure}) {
    _lastHost = host.trim();
    _lastPort = port;
    _lastNickname = nickname.trim();
    _lastSecure = secure;
  ...
  void join(String channel) {
    final n = channel.trim();
    if (!connected || n.isEmpty) return;
    rooms.putIfAbsent(n, () => ChatRoomModel(n));
    activeRoom = n;
    rooms[n]!.unread = 0;
    client.join(n);
    notifyListeners();
  ...
  void openPrivate(String nickname) {
    ...
    if (!connected || n.isEmpty) return;
    rooms.putIfAbsent(n, () => ChatRoomModel(n, privateChat: true));
    activeRoom = n;
    rooms[n]!.unread = 0;
    notifyListeners();
  }
  ...
  void closeRoom(String name) {
    ...
    final r = rooms[name];
    if (r == null) return;
    if (!r.privateChat && connected) client.part(name);
    rooms.remove(name);
    if (activeRoom == name) activeRoom = rooms.isEmpty ? null : rooms.keys.first;
    notifyListeners();
  }

  void changeNick(String nickname) {
    final n = nickname.trim();
    if (connected && n.isNotEmpty) client.changeNick(n);
  }
  ...
  void sendMessage(String text) {
    ...
    rooms[target]?.messages.add(IrcMessage(':local!local@local', 'local', 'PRIVMSG', [target, value]));
    notifyListeners();
  }

  void sendPrivate(String target, String text) {
    final t = target.trim(), v = text.trim();
    if (!connected || t.isEmpty || v.isEmpty) return;
    openPrivate(t);
    client.message(t, v);
    rooms[t]?.messages.add(IrcMessage(':local!local@local', 'local', 'PRIVMSG', [t, v]));
    notifyListeners();
  }

  void sendNotice(String target, String text) {
    if (connected && target.trim().isNotEmpty && text.trim().isNotEmpty) client.notice(target.trim(), text.trim());
  }

  void sendAction(String target, String text) {
    if (connected && target.trim().isNotEmpty && text.trim().isNotEmpty) client.message(target.trim(), '\u0001ACTION ${text.trim()}\u0001');
  }

  void toggleIgnore(String nickname) {
    final n = nickname.trim().toLowerCase();
    if (n.isEmpty) return;
    if (!ignoredUsers.add(n)) ignoredUsers.remove(n);
    notifyListeners();
  }
  ...
  void handleMessage(IrcMessage message) {
    ...
    if (message.command == 'JOIN' && message.params.isNotEmpty && sender != null) {
      final channel = message.params.last;
      final room = rooms.putIfAbsent(channel, () => ChatRoomModel(channel));
      room.addUser(sender);
      if (_sameNick(sender, _lastNickname)) {
        activeRoom ??= channel;
        room.unread = 0;
      }
    }

    if (message.command == 'PART' && message.params.isNotEmpty && sender != null) {
      final channel = message.params.first;
      rooms[channel]?.removeUser(sender);
      if (_sameNick(sender, _lastNickname)) {
        rooms.remove(channel);
        if (activeRoom == channel) activeRoom = rooms.isEmpty ? null : rooms.keys.first;
      }
    }

    if (message.command == 'QUIT' && sender != null) {
      for (final room in rooms.values) {
        room.removeUser(sender);
      }
    }

    if (message.command == 'NICK' && sender != null && message.params.isNotEmpty) {
      final newNick = message.params.last;
      for (final room in rooms.values) {
        room.renameUser(sender, newNick);
      }
      if (_sameNick(sender, _lastNickname)) {
        _lastNickname = newNick;
        status = 'Nickname: $newNick';
      }
    }

    if (message.command == 'KICK' && message.params.length >= 2) {
      final channel = message.params.first;
      final kicked = message.params[1];
      rooms[channel]?.removeUser(kicked);
      if (_sameNick(kicked, _lastNickname)) {
        rooms.remove(channel);
        if (activeRoom == channel) activeRoom = rooms.isEmpty ? null : rooms.keys.first;
        status = 'Expulsado de $channel';
      }
    }

    if (message.command == 'TOPIC' && message.params.isNotEmpty) {
      final room = rooms[message.params.first];
      if (room != null) room.topic = message.trailing;
    }

    if (message.command == '332' && message.params.length >= 2) {
      final channel = message.params[1];
      final room = rooms.putIfAbsent(channel, () => ChatRoomModel(channel));
      room.topic = message.trailing;
    }

    if (message.command == 'PRIVMSG' || message.command == 'NOTICE') {
      _handleTextMessage(message, sender);
    }

    if (message.command == '353' && message.params.length >= 3) {
      final channel = message.params[2];
      final room = rooms.putIfAbsent(channel, () => ChatRoomModel(channel));
      final names = message.trailing.trim();
      if (names.isNotEmpty) {
        for (final raw in names.split(RegExp(r'\s+'))) {
          if (raw.isEmpty) continue;
          final match = RegExp(r'^([~&@%+]+)(.+)$').firstMatch(raw);
          final prefixes = match?.group(1) ?? '';
          final nick = match?.group(2) ?? raw;
          if (nick.isNotEmpty) room.addUser(nick, prefixes: prefixes);
        }
      }
    }
  ...
  void _applyUserModes(IrcMessage message) {
    ...
    const modesWithArgument = {'q', 'a', 'o', 'h', 'v', 'b', 'e', 'I', 'k', 'l'};
    ...
    for (final mode in message.params[1].split('')) {
      ...
      if (!modesWithArgument.contains(mode)) continue;
      if (arg >= message.params.length) break;
      final value = message.params[arg++];
      final prefix = modeMap[mode];
      if (prefix == null) continue;

      var prefixes = room.users[value]?.prefixes ?? '';
      if (adding && !prefixes.contains(prefix)) prefixes += prefix;
      if (!adding) prefixes = prefixes.replaceAll(prefix, '');
      room.setUserPrefixes(value, prefixes);
    }
  }
