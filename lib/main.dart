...
class _IrcHomePageState extends State<IrcHomePage> {
  ...
  Widget _chat() { final msgs = active.messages; return ListView.builder(padding: const EdgeInsets.all(12), itemCount: msgs.length, itemBuilder: (_, i) { final m = msgs[i]; final isMe = m.nick == nick.text; return Padding(padding: const EdgeInsets.symmetric(vertical: 4), child: RichText(text: TextSpan(children: [TextSpan(text: '${m.nick ?? 'Sistema'}: ', style: TextStyle(fontWeight: FontWeight.bold, color: isMe ? Colors.indigoAccent : Colors.white70)), TextSpan(text: m.trailing, style: const TextStyle(color: Colors.white))]))); }); }

  Widget _usersPanel() { final users = active.users.toList()..sort(); return Container(width: 125, decoration: const BoxDecoration(border: Border(left: BorderSide(color: Colors.white12))), child: ListView(padding: const EdgeInsets.all(8), children: [const Text('USUARIOS', style: TextStyle(fontWeight: FontWeight.bold)), const SizedBox(height: 8), for (final u in users) Padding(padding: const EdgeInsets.symmetric(vertical: 3), child: Text('• $u'))])); }
